		...
		if (message.member.user.id == '313351494361677845' || message.member.roles.cache.some(role => role.id == staffRoleID)) {
			...
			for (let k = 1; k <= countersData.bansSeq; k++) {
				...
				if (gbannedData) {
					...
					if (gbannedData.gunbanDate > Date.now()) {
					...
					} else if (gbannedData.gunbanDate < Date.now() && await memberTargetFunction.roles.cache.some(role => role.id == gBannedRole)) {
			...
			message.channel.send("**Successfully started all the processes**")
		}else{
