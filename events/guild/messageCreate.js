...
module.exports = {
    async execute(message, client, Discord) {
        ...
        if(message.author.id == '555955826880413696' && message.embeds[0]){
            if(message.embeds[0].fields[0]?.value.toLowerCase().startsWith('type ') || message.embeds[0].fields[0]?.name.toLowerCase().startsWith('type')) return client.emit('erpgEvent', client, message, Discord, false)
